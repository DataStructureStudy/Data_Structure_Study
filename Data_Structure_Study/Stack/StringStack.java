...
public class StringStack {
        ...
        //스택 메서드
        char push(char data) throws StringStack.OverflowIntStackExcetion {
            if(this.top>=this.max){ // == 부호도 가능한데 등가부호를 사용하는 이유가 있음
                throw new StringStack.OverflowIntStackExcetion();
            }
            return this.stack[++this.top] = data;
        }

        char pop() throws StringStack.EmptyIntStackExcetion {
            if(this.top<0){
                throw new StringStack.EmptyIntStackExcetion(); //C언어에서 exit()함수 쓰는거랑 비슷한거
            }
            return this.stack[this.top--];
        }
        //상단 data추출
        char peek() throws StringStack.EmptyIntStackExcetion {
            if(this.top < 0){
                throw new StringStack.EmptyIntStackExcetion();
            }
            return this.stack[this.top];
        }
        //인덱스검색
        ...
        void dump() throws StringStack.EmptyIntStackExcetion {
            if(this.top<0){
                throw new StringStack.EmptyIntStackExcetion();
            }
            else{
